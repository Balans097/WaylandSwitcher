    ...
    if n == 3:
      ...
      if leftDown and not leftWasDown:
        ...
        if travelPx >= MouseDragThresholdPx:
          ...
          # Обычный клик (не drag).
          #
          # РАНЬШЕ здесь стояло "elif needTrackMouse.load()" — буфер и
          # выделение (selActive) сбрасывались ТОЛЬКО если клик пришёлся
          # на момент накопления буфера набора текста. Но самый обычный
          # клик — это переключение фокуса мышью в другое поле ввода или
          # в другое приложение, а именно в этот момент needTrackMouse,
          # как правило, уже false (пользователь ничего не печатал прямо
          # перед кликом). В таком случае старое условие не выполнялось,
          # и если к этому моменту где-то раньше уже было выставлено
          # selActive=true (drag/Ctrl+A/Shift+навигация) и коррекция так
          # и не была вызвана — оно оставалось true и после клика.
          #
          # Следующее нажатие клавиши коррекции в НОВОМ, уже другом поле
          # ошибочно попадало в ветку «коррекция выделения» вместо
          # обычной построчной коррекции: emitSelectionCorrection делала
          # Ctrl+C уже в новом месте. Если там ничего не было выделено —
          # коррекция просто молча прерывалась по маркеру (снаружи это
          # выглядело как «раскладка не исправляется»). Если же в новом
          # поле что-то оказывалось выделено (например, поле само
          # выделяет всё своё содержимое при получении фокуса — типичное
          # поведение однострочных полей вроде адресной строки) —
          # программа конвертировала и вставляла ЭТОТ чужой, не
          # относящийся к делу текст поверх содержимого нового поля —
          # именно это и описывалось как «вставляется конвертированный
          # буфер обмена от какого-то другого поля».
          #
          # Исправление: сбрасывать буфер и selActive на КАЖДОМ обычном
          # клике, а не только во время накопления буфера — клик считается
          # универсальным признаком «пользователь сменил место работы»,
          # будь то позиционирование курсора в том же поле или переход
          # в другое.
          needTrackMouse.store(false)
          needClearKeyBuf.store(true)
          logMsg(false, "mouse click: buffer/selection clearing queued", stdOnly = true)
  ...
  while not stopAndExit.load():
    ...
    if read(kfd, addr ie, sizeof(ie)) == sizeof(ie):
      ...
      if ie.ie_type == EV_KEY and ie.value in [0'i32, 1]:
        ...
        # Tab / Alt — используются в переключении окон/полей клавиатурой
        # (Alt+Tab, Tab между полями формы). У нас нет доступа к событиям
        # фокуса окна от compositor'а, поэтому это единственный доступный
        # прокси-признак того, что фокус мог смениться без клика мышью и
        # без явного Escape/Enter. Без этого сброса «залипшее» selActive,
        # выставленное в одном окне (drag/Ctrl+A/Shift-навигация) и не
        # использованное для коррекции, переживало Alt+Tab и ошибочно
        # подхватывалось следующим нажатием клавиши коррекции уже в другом
        # окне — с теми же последствиями, что и в случае с кликом мыши
        # (см. комментарий в mouseThreadFn): либо коррекция молча
        # прерывалась (маркер в CLIPBOARD), либо, что хуже, вставлялся
        # исправленный текст поверх чужого выделения (например,
        # автовыделения содержимого поля при получении им фокуса).
        const FocusChangeKeys = {15'u16,  ## TAB
                                  56'u16,  ## LEFTALT
                                  100'u16} ## RIGHTALT
        if ie.value == 1 and ie.code in FocusChangeKeys:
          if selActive:
            logMsg(false, "выделение сброшено (Tab/Alt — вероятное переключение окна)",
                   stdOnly = true)
          selActive = false
          selCount  = 0

        # Возобновление обычного набора текста поверх «залипшего» selActive.
        #
        # Если пользователь переключился в другое поле/приложение способом,
        # который не попал ни под один из детекторов выше (клик мыши,
        # Tab/Alt, Escape/Enter, стрелка без Shift) — например, фокус сменил
        # сам оконный менеджер, или переключение произошло от третьей
        # стороны (не через физическую клавиатуру/мышь этой программы), —
        # то первым надёжным сигналом «пользователь работает в новом месте»
        # становится обычный набор текста: набор буквы/цифры без Ctrl
        # ГАРАНТИРОВАННО означает, что предыдущее выделение (если оно вообще
        # было) уже не актуально — ввод символа в любом текстовом поле либо
        # заменяет выделение, либо просто продолжает набор без выделения.
        # code != cfg.keyRpl и not ctrlHeld исключают саму клавишу коррекции
        # и события вида Ctrl+A/Ctrl+<буква>, у которых уже есть собственная
        # обработка выше (Ctrl+A взводит selActive именно на этом событии,
        # проверка ctrlHeld здесь гарантирует, что мы не затрём её результат).
        if selActive and ie.value == 1 and ie.code in Letters and
           ie.code != cfg.keyRpl and not ctrlHeld:
          logMsg(false, "выделение сброшено (обычный набор текста поверх старого выделения)",
                 stdOnly = true)
          selActive = false
          selCount  = 0
