...
proc prepareBuffer*(keyBuf: var seq[InputEvent]; keyRpl: uint16) =
  ...
  # Каждый Backspace должен удалить ровно один предыдущий символ из буфера.
  # Но если непосредственно перед удаляемым символом стоит Shift (то есть
  # символ был набран с зажатым Shift — заглавная буква), удаление одной
  # лишь буквы оставило бы «голый» Shift висящим перед уже другим символом,
  # ошибочно делая его тоже заглавным при повторном воспроизведении.
  # Поэтому при обнаружении Shift перед удаляемым символом — Shift не
  # удаляется отдельно, а «передвигается» на одну позицию назад (на место
  # удаляемого символа, к новому соседу) через res[^2] = res[^1], после
  # чего лишняя (теперь продублированная) копия снимается через pop().
  # Если до сдвига в буфере меньше двух элементов — двигать некуда, Shift
  # просто остаётся как был (защита от выхода за границы).
  for ev in buf:
