...
##
## История изменений:
##   1.2 — исправлено обнаружение Ctrl+A, добавленное в 1.1: оно не
##         срабатывало, если пользователь отпускал Ctrl раньше A (что
##         оказалось обычным порядком отжатия на практике) — детектор
##         проверял key-up клавиши A при ctrlHeld, а ctrlHeld к этому
##         моменту уже был сброшен предыдущим событием LEFTCTRL up.
##         Перенесено на key-down клавиши A: порядок НАЖАТИЯ в сочетании
##         Ctrl+A гарантирован (Ctrl всегда нажат раньше A), а порядок
##         отжатия — нет (2026-06-28)
##   1.1 — добавлена поддержка Ctrl+A («выделить всё») как третьего
##         способа создания выделения для коррекции раскладки, наравне
##         с drag мышью и Shift+навигационными клавишами. До этого
##         исправления Ctrl+A не распознавался вообще: selActive
##         оставался false, и нажатие клавиши коррекции после Ctrl+A
##         ничего не делало. Заодно исправлена смежная проблема: любая
##         буква при зажатом Ctrl (Ctrl+A, Ctrl+S, Ctrl+Z и т.д.) могла
##         попасть в keyBuf обычного режима как будто пользователь её
##         напечатал, потому что проверка `ie.code in Letters` не
##         учитывала состояние Ctrl — теперь учитывает (2026-06-28)
##   1.0 — первый стабильный релиз: коррекция выделенного текста (мышью
##         и Shift+стрелками/Home/End/PageUp/PageDown) подтверждена
##         работающей, включая многострочные выделения. По сравнению
##         с 0.11:
##           • Убран лишний хвостовой перевод строки в скорректированном
##             тексте — wl-paste --no-newline иногда всё же отдаёт
##             хвостовой \r\n/\n, который раньше попадал в вставляемый
##             текст; теперь он отрезается сразу после чтения CLIPBOARD
##             (только хвост, внутренние переводы строк не трогаются).
##           • Существенно расширены комментарии по всему коду —
##             обоснование решений, тонкие места (учёт Shift при
##             Backspace, автоопределение направления перекодировки,
##             порядок шагов коррекции выделения и почему он именно
##             такой) теперь объяснены подробно прямо в исходниках.
##           • reinstall.sh переписан: теперь только устанавливает уже
##             собранный бинарник ./WaylandSwitcher, передавая установку
##             встроенной команде 'wayland-switcher -R' (runReinstall в
##             setup.nim) вместо ручного stop/cp/chmod/chown/start — это
##             даёт атомарную замену бинарника, резервную копию и
##             автоматический откат при ошибке. Компиляция (отдельной
##             командой 'nim c --threads:on -d:release WaylandSwitcher.nim')
##             в скрипт сознательно не входит — выполняется самостоятельно
##             перед запуском reinstall.sh (2026-06-28)
