...
# ── Пороги распознавания жестов мыши ─────────────────────────────────────────
const
  ## Минимальное суммарное смещение курсора (в «попугаях» протокола
  ## /dev/input/mice, примерно равно пикселям при стандартной чувствительности)
  ## за время удержания левой кнопки, начиная с которого жест считается
  ## протаскиванием (drag → выделение текста), а не обычным кликом.
  ## Меньше порога — клик: буфер набора текста сбрасывается, как раньше.
  ## Больше порога — drag: выставляется mouseSelDone, главный цикл считает
  ## это завершённым выделением текста мышью.
...
const
  ...
  ## Клавиши-стрелки и навигационные клавиши, при удержании Shift
  ## образующие выделение текста.
  SelectionKeys* = {103'u16, 105, 106, 108,  ## UP, LEFT, RIGHT, DOWN
                    102'u16, 107, 104, 109}   ## HOME, END, PAGEUP, PAGEDOWN

  ## Левый Ctrl (29) и правый Ctrl (97).
  Ctrls* = {29'u16, 97}
...
type
  ...
  ## Инструмент буфера обмена для работы с выделенным текстом.
  ClipboardTool* = enum
    ClipAuto      ## автоопределение (wl-paste → xclip → xsel)
    ClipWayland   ## wl-paste / wl-copy (для Wayland-сессий)
    ClipXclip     ## xclip (для X11-сессий)
    ClipXsel      ## xsel (для X11-сессий)
  ...
  AppConfig* = object
    ...
    clipTool*:    ClipboardTool ## инструмент буфера обмена для коррекции выделения
