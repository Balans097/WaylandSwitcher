...
proc emitEvents*(vfd: cint; keyBuf: seq[InputEvent];
  ...
  # Отслеживаем состояние клавиш «нажата/отпущена» по мере накопления
  # событий — нужно для страховки в самом конце (см. ниже, после группы 3).
  var heldKeys = initTable[uint16, bool]()
  ...
  template push(ev: InputEvent) =
    ieBuf.add(ev)
    t.tv_usec += 200
    if ev.ie_type == EV_KEY:
      if ev.value == 1:
        heldKeys[ev.code] = true
      elif ev.value == 0:
        heldKeys.del(ev.code)
  ...
  # ── Страховка: принудительно отпускаем всё, что осталось «нажатым» ──────
  # keyBuf в группе 3 — это события ИЗ ЖИВОГО ВВОДА пользователя, повторно
  # проигранные как есть. prepareBuffer (см. выше в этом модуле) старается
  # довести их до сбалансированного вида (каждому down — свой up), но
  # гарантировать это на 100% для абсолютно всех комбинаций — от «Shift
  # ещё зажат физически в момент отпускания клавиши коррекции» (сценарий
  # «Shift (с удержанием) + PAUSE» из README — Shift, набранный перед этим
  # словом, и Shift, использованный для самой комбинации коррекции, могут
  # быть одним и тем же удержанием, при этом ShiftUp в буфере ещё попросту
  # не появился, потому что физически клавиша не отпущена) до менее
  # очевидных случаев — очень легко не учесть.
  #
  # Если хотя бы одна такая клавиша (обычно Shift) уйдёт в виртуальную
  # клавиатуру нажатой и без парного отпускания, композитор будет считать
  # её зажатой НАВСЕГДА (наше виртуальное устройство — не более чем ещё
  # один источник событий для общего состояния клавиатуры seat'а; раз мы
  # сами никогда не пришлём для него key-up, реальное отпускание Shift
  # пользователем на физической клавиатуре эту виртуальную «половину» не
  # исправит) — снаружи это выглядит как «залипший Shift»: весь ввод идёт
  # заглавными буквами, Caps Lock/Shift это не лечат, а клик в файловом
  # менеджере ведёт себя как Shift+клик.
  #
  # Поэтому вместо того чтобы пытаться перечислить и закрыть все мыслимые
  # частные случаи в prepareBuffer, здесь — в самом конце, после всех трёх
  # групп событий — принудительно отпускаем на виртуальном устройстве
  # ЛЮБУЮ клавишу, которая по нашему собственному подсчёту (heldKeys)
  # осталась «нажатой». Реального физического состояния пользователя это
  # не портит: то, что происходит на физической клавиатуре, отслеживается
  # отдельно и независимо в главном цикле (WaylandSwitcher.nim) через
  # shiftHeld/ctrlHeld и не имеет отношения к тому, что мы сами отправили
  # на СВОЁ виртуальное устройство.
  var stuckKeys: seq[uint16]
  for code in heldKeys.keys:
    stuckKeys.add(code)
  for code in stuckKeys:
    push InputEvent(ie_type: EV_KEY, code: code, value: 0, time: t)
    logMsg(false,
      "страховка: принудительно отпускаем застрявшую клавишу " & keyNameSafe(code),
      stdOnly = true)
